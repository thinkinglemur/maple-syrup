...
# ─────────────────────────────────────────────────────────────
#  init.sh  — generated by maple-syrup
...
# Print the line number if something unexpected causes an exit
trap 'echo -e "\n${RED}✖ init.sh failed at line $LINENO — check output above.${NC}\n"' ERR
...
# PM and PMX are set by the Node.js version check above
TS_INIT
...
EXPRESS
  ...
  # Sentry + GA — conditional on framework
  case "$CHOSEN_FRAMEWORK" in
    "Next.js"|"Remix"|"NestJS")
      cat >> "$out" <<'TS_EXTRAS'

step "Installing observability packages..."
$PM add @sentry/nextjs || warn "Sentry install failed — add manually later"
ok "Sentry installed"

# Sentry config stubs
cat > sentry.client.config.ts <<SENTRY_CLIENT
...
cat > sentry.server.config.ts <<SENTRY_SERVER
import * as Sentry from "@sentry/nextjs";
Sentry.init({
  dsn: process.env.NEXT_PUBLIC_SENTRY_DSN,
  tracesSampleRate: 1.0,
...
# Analytics stub
mkdir -p src/lib
cat > src/lib/analytics.ts <<ANALYTICS_EOF
...
TS_EXTRAS
      ;;
    "Express"|"Fastify")
      cat >> "$out" <<'EXPRESS_SENTRY'

step "Installing observability packages..."
$PM add @sentry/node || warn "Sentry install failed — add manually later"
mkdir -p src/lib
cat > src/lib/analytics.ts <<ANALYTICS_EOF
// Google Analytics — add gtag snippet to your HTML template
...
pip install ${framework_pkg} sentry-sdk python-dotenv || { error "pip install failed"; exit 1; }
...
_append_compose() {
  ...
  # Write the app service — only include depends_on if we have a DB or cache
...
MEMCACHED
  ...
  # Volumes — only declare volumes that are actually used
  if [[ -n "$CHOSEN_DATABASE" && -n "$CHOSEN_CACHE" ]]; then
    cat >> "$out" <<'VOLS_BOTH'
